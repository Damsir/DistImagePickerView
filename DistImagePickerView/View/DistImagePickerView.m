...
#import "DistImagePickerConst.h"
#import "DistImagePickerManager.h"
#import "TZImagePickerController.h"
#import "MWPhotoBrowser.h"

static NSInteger countOfRow;

@interface DistImagePickerView () <UICollectionViewDelegate,UICollectionViewDataSource,TZImagePickerControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate,MWPhotoBrowserDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;

@property (nonatomic, copy) DistImagePickerHeightBlock block;

@property (nonatomic, copy) DistSelecttImageBackBlock backBlock;
...
@implementation DistImagePickerView
...
- (void)setup {
    ...
    _type = DistImageTypePhotoAndCamera;
    _showDelete = YES;
    _showAddButton = YES;
...
- (void)configureCollectionView {
    ...
    [_collectionView registerClass:[DistImagePickerCell class] forCellWithReuseIdentifier:NSStringFromClass([DistImagePickerCell class])];
...
- (void)setPreShowMedias:(NSArray *)preShowMedias {
    ...
    for (id object in preShowMedias) {
        DistImagePickerModel *model = [DistImagePickerModel new];
        ...
        } else if ([object isKindOfClass:[DistImagePickerModel class]]) {
            model = object;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DistImagePickerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([DistImagePickerCell class]) forIndexPath:indexPath];
        ...
        cell.DistClickDeleteButton = ^(NSIndexPath *cellIndexPath) {
            ...
            if (!_allowMultipleSelection) {
                ...
                for (NSInteger idx = 0; idx < _selectedImageModels.count; idx++) {
                    ...
                    if ([((DistImagePickerModel *)_selectedImageModels[idx]) isEqual:model ]) {
                ...
                for (NSInteger idx = 0; idx < _selectedVideoModels.count; idx++) {
                    ...
                    if ([((DistImagePickerModel *)_selectedVideoModels[idx]) isEqual:model]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _mediaArray.count) {
        ...
        for (DistImagePickerModel *model in _mediaArray) {
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto{
    ...
    for (NSInteger index = 0; index < assets.count; index++) {
        ...
        [[DistImagePickerManager manager] getMediaInfoFromAsset:asset completion:^(NSString *name, id pathData) {
            ...
            if (!_allowMultipleSelection) {
                //用数组是否包含来判断是不成功的。。。
                for (DistImagePickerModel *md in _selectedImageModels) {
                    // 新方法
                    if ([md isEqual:model] ) {
                        [temp addObject:md];
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingVideo:(UIImage *)coverImage sourceAssets:(id)asset {
    [[DistImagePickerManager manager] getMediaInfoFromAsset:asset completion:^(NSString *name, id pathData) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!_allowMultipleSelection) {
                //用数组是否包含来判断是不成功的。。。
                for (DistImagePickerModel *tmp in _selectedVideoModels) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if ([mediaType isEqualToString:@"public.movie"]) {
        ...
        [[DistImagePickerManager manager] getVideoPathFromURL:videoAssetURL PHAsset:asset enableSave:NO completion:^(NSString *name, UIImage *screenshot, id pathData) {
            DistImagePickerModel *model = [[DistImagePickerModel alloc] init];
        ...
        [[DistImagePickerManager manager] getImageInfoFromImage:image PHAsset:asset completion:^(NSString *name, NSData *data) {
            DistImagePickerModel *model = [[DistImagePickerModel alloc] init];
            model.image = image;
            model.name = name;
